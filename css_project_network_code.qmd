...
#previous collaborators tibble
prev_collabs <- read.csv()

#filtering out previously collaborated
filter_test <- matching_test %>% 
  anti_join(prev_collabs, by = c("name_from" = "names1", "name_to" = "names2")) %>% 
  anti_join(prev_collabs, by = c("name_from" = "names2", "name_to" = "names1"))

#generating a list of each individuals matches
matching_list <- matching_test %>% 
  group_by(name_from) %>% 
  arrange(desc(weight)) %>% 
  summarize(neighbors = list(name_to), .groups = "drop")

#adding in the top matching collaborator for each individual
matching_list <- matching_list %>% 
  mutate(collab_number = map(neighbors, length)) %>% 
  mutate(top_neighbor = map(neighbors, first)) %>% 
  unnest(top_neighbor) %>% 
  unnest(collab_number) %>% 
  relocate(top_neighbor, .before = "neighbors")

#keeping only matches where they match to each other
match_mates <- matching_list %>%
  select(name_from, top_neighbor)
  rowwise() %>%
  mutate(pair_key = paste(sort(c(names1, names2)), collapse = "_")) %>%
  ungroup() %>%
  group_by(pair_key) %>% 
  filter(n() > 1) %>% 
  ungroup() %>% 
...
Visuals (to be built up for paper)
