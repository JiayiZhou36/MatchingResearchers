...
network_clusters <- cluster_louvain(network_graph_modified, resolution = 3)
...
matching_list <- matching_test %>% 
  ...
  arrange(desc(weight)) %>% 
  mutate(neighbors = list(name_to), .groups = "drop")
...
match_mates <- matching_list %>%
  ...
  rowwise() %>%
  mutate(pair_key = paste(sort(c(name_from, top_neighbor)), collapse = "_")) %>%
  ungroup() %>%
  group_by(pair_key) %>%
  filter(n() > 1) %>% 
  ungroup() %>% 
