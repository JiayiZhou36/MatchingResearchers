...
Keeping ties based on weights and creating clustering
...
#running a test of various resolutions to look at stable community sizes (should run this section only if you want to change the resolution parameter)
...
write.csv(edges_network, here("edges_network.csv"))
...
write.csv(nodes_network, here("nodes_network.csv"))
...
similarity_output <- match_similarity_mates %>% 
  select(full_name_from, discipline_from, full_name_to, discipline_to, weight) %>% 
  rename("Researcher 1" = full_name_from, 
         "Researcher 2" = full_name_to,
         "Discipline 1" = discipline_from,
         "Discipline 2" = discipline_to,
         "Cosine Similarity" = weight) %>% 
  slice_sample(n = 10)
print(similarity_output)

similarity_output_tt <- tt(similarity_output)

save_tt(similarity_output_tt, here("tinytable.png"), overwrite = TRUE)
```


Preliminary Network Visuals
