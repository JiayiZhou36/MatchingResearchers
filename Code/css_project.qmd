...
similarity_data <- read.csv(here("data/professor_similarity_matrix.csv"))

#creating a code from names to numbers
name_code <- setNames(similarity_data$X, 1:nrow(similarity_data))

#creating adjacency matrix
similarity_data <- similarity_data %>% 
  select(-X) #removing name column
adjacency_matrix <- as.matrix(similarity_data)
#removing self-loops
diag(adjacency_matrix) <- 0

#Creating network
network <- graph_from_adjacency_matrix(adjacency_matrix, weighted = TRUE)
```
...
ggraph(network_modified, layout = "stress") +
  geom_edge_link() +
  geom_node_point()
```
